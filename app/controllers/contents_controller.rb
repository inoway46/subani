  ...
  def create
    if params[:content][:master_id]
      ...
      @master_ids.each do |master_id|
        ...
        @content = current_user.contents.create!(title: master.title, media: master.media, url: master.url, stream: master.stream, registered: true, new_flag: true, episode: master.episode, master_id: master.id)
